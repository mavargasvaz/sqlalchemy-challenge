...
def welcome():
    ...
    return (
        f"Welcome to the Mike's TEMPERATURE APP!<br/>"
        f"<br/>"
        f"<br/>"
        f"Available Routes:<br/>"
        f"Precipitation: /api/v1.0/precipitation<br/>"
        f"List of Stations: /api/v1.0/stations<br/>"
        f"Temperature for most active Station: /api/v1.0/tobs<br/>"
...
def tobs():
    ...
    for date, tobs in queryresult:
        ...
        tobs_dict["Date"] = date
        tobs_dict["Tobs"] = tobs
        tobsactive.append(tobs_dict)

    return jsonify(tobsactive)

 # Query at specified start, calculate TMIN, TAVG, and TMAX for all the dates greater than or equal to the start date.
